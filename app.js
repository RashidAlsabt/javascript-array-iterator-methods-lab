...
let veryOldInventors = inventors.filter((element)=>{
...
let inventorNames = inventors.map((element, index)=>{
    return {firstname: element.first, lastName:element.last}
});
...
let sortedByBirthYear = inventors.sort((element1,element2)=>{
...
let inventorNamedAda = inventors.find((element)=>{
    if (element.first === 'ada'){
        return true;
    } else {
        return false;
    }
...
let firstLast = people.map((element)=>{
...
let isAdultPresent = devs.some((element)=>{
    const year = new Date().getFullYear()
    const age = year - element.year
    return age > 19
});
...
let isEveryone19OrOlder = devs.every((element)=>{
    const year = new Date().getFullYear()
    const age = year - element.year
    return age >= 19 
});
